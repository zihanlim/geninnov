...
| `docs/MASTER_MAP.md` | **The single index of every part of the research process** — data sources, layers, formulas, selection, sizing, publication, frontend surfaces, verification, with links to every source doc. Compiled 2026-08-02; ARCHITECTURE.md wins on any conflict. The animated `/datamap` page in the app is the live version of this map |
...
| `scripts/watch_phrases.py` | **Is a candidate narrative approaching the point where it can be named?** Read-only; writes nothing, promotes nothing. Reports the state one step BEFORE the board — a narrative in the headlines that has not repeated enough to clear the document floor. Imports `load_market_corpus`, `document_floor` and `phrases_in` rather than reimplementing any of them: the first draft read `market_news` directly and was wrong three ways (PostgREST's 1000-row cap, grouping on the FETCH day, no dedupe), each of which moves the denominator every number here is a ratio against. Also prints an ADR-0155 comparability warning when its corpus has broken away from the board's |
...
| `frontend/app/datamap/page.tsx` + `components/datamap/dataMap.ts` | L6: the animated system map — sources, L0–L8, Supabase tables, frontend surfaces, in four swimlanes. Pure data in `dataMap.ts`; the renderer never hard-codes a node. Cross-cutting like `/method` and `/facts` so it is NOT in the top-bar nav |
...
| `frontend/components/RibbonControl.tsx` | The shared *look* of a TopBar tool control (`Ask`, `Live news`) — label gated to `wide`, `aria-label` always. Behaviour is deliberately NOT here. In-bar styling uses the `--header-*` light-on-navy tokens (the masthead is solid `--logo-plate`, [ADR-0223](docs/adrs/0223-the-navy-is-the-brand-ground.md)); the panels it opens stay light cards |
