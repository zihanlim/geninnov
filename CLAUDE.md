...
| `frontend/components/NarrativeTrends.tsx` | The trends board on `/` — share of voice over time per narrative, plus the emerging-and-uncovered shortlist. Written against [ADR-0126](docs/adrs/0126-a-chart-without-a-value-axis-is-a-shape.md) (value axis, direct labels + leaders, figures table) on the **validated** `--series-1..5` palette. Two palette slots sit below 3:1 contrast, legal only with the labels and table — `narrative-trends.test.tsx` asserts they stay |
