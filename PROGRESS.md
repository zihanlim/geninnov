...
_Last updated: 2026-07-22 (T27 architecture docs sync + ADRs 0016–0018)_
...
| 2026-07-22 | **T27: architecture docs synced + ADRs 0016–0018.** `ARCHITECTURE.md` mermaid diagram updated to remove `backend/agents/research_agent.py` (T17 retired the LangGraph skeleton) and add `backend/derivations/` subgraph, `backend/services/{pipeline_runs,portfolio,exposure,risk_engine,hype_calculator}.py`, the `pipeline_runs` + `portfolio_cumulative_return` tables, and the L7 status / portfolio / research read-models. `## Layers` and `## Supabase Tables` tables now reflect the derivation JSONB columns and the new tables. `## Feature Checklist` ticks off signed weights, cumulative compounding, provenance, derivation columns, Q1 advisory strictness, taxonomy seam, `elapsed_days` plumbing, `abs(corr)` normalization, e2e tests, verification matrix, residual risk, lineage matrix, and runtime execution graph; leaves open migrations 010/011 deployed, cron owned externally, and theme discovery shadow mode. Three new ADRs: 0016-signed-weights-portfolio-accounting, 0017-since-inception-cumulative-performance, 0018-provenance-read-model-seam. Doc-only change; `cd frontend && npm run build` re-verified clean.
