...
The append-only record of what each `/loop` firing did. **Split out of
[`GOAL.md`](GOAL.md) on 2026-07-27**, where it had grown to 5,035 of that file's
5,452 lines — 93% of a document the loop re-reads on every firing.

**If you followed a link here from an ADR:** several ADRs cite iterations as
`[Iteration 21](docs/GOAL.md)` or `[Iteration 24](../GOAL.md)`. ADRs are immutable once
written, so those links still point at `GOAL.md`; the iterations they mean are in this
file. Search for `### Loop iteration <N>`.

**Ordering is newest first.** Iteration 103 is at the top, iteration 2 at the bottom.
Append new entries directly under this header, not at the end.

**This file is history, not instruction.** Nothing here binds current work. The standing
brief — the deliverables, the four bars, the loop mandate, the operating principles, the
live candidate gaps and the hard constraints — is in [`GOAL.md`](GOAL.md), and that is
the file to read before acting. An entry here explains why something is the way it is; it
