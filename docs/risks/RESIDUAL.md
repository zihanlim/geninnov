...
| R5 | Theme discovery is shadow-mode-only until manual approval | `theme_discovery.py` output is not promoted into `themes` until reviewed | `scripts/theme_discovery.py` + ops | Accepted | **Updated 2026-07-23:** `theme_discovery.py` now implements the LDA∩embedding agreement and **persists** Tier 2/3 candidates to the `discovered_themes` shadow table (migration 021) instead of printing to stdout. Auto-promotion into `themes` remains intentionally manual |
