...
| R7 | 2 pre-existing test failures in `tests/backend/test_brave_client.py` (Windows cp1252) | CI signal noise; not introduced by T1–T27 work | `tests/backend/test_brave_client.py` | Accepted | Re-encoding to `utf-8` is opportunistic; tracked here so the failures are not mistaken for regressions |
