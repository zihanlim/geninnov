...
// ── expanded-panel grid ─────────────────────────────────────────────────────────
//
// Row 1 is the argument and its rebuttal. The EdgeScore decomposition moved down to
// r2c1 to make room, taken from the Stitch comp's answer layout (claim beside its
// offset rather than under it). Both columns are ~310px either way, so this costs no
// width — only adjacency changes.
describe("the expanded panel reads the claim beside its rebuttal", () => {
  ...
  it("puts the counter-thesis beside the thesis, not under it", () => {
    const html = render({ open: true });
    expect(cellOf(html, "Thesis")).toBe("r1c1");
    expect(cellOf(html, "Counter-thesis")).toBe("r1c2");
  });

  it("moves the EdgeScore decomposition down rather than dropping it", () => {
    expect(cellOf(render({ open: true }), "Why long")).toBe("r2c1");
  });

  it("leaves every other card where it was", () => {
    // `Sizing` and `Factor exposure` carry NO row/col and are placed by grid
    // auto-flow into whatever is free — so pinning the explicit ones is what stops a
    // future edit from silently reflowing them.
    const html = render({ open: true });
    for (const [title, at] of [
      ...
      expect(cellOf(html, title), title).toBe(at);
    }
  });

  it("still renders the auto-placed cards", () => {
    const html = render({ open: true });
    expect(html).toContain("Sizing — conviction");
    expect(html).toContain("Factor exposure");
  });
});
